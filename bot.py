...
async def start_night(chat_id: int):
    ...
    # Мафия видит только мирных, шерифа и доктора. Своих товарищей по команде она не видит (правила игры)
...
async def cb_night(callback: CallbackQuery):
    ...
    try:
        if chat_id not in games or games[chat_id].phase != "night":
            return await callback.answer("⚠️ Сейчас не ночная фаза.", show_alert=True)
...
async def cb_vote(callback: CallbackQuery):
    ...
    if chat_id not in games or games[chat_id].phase != "voting":
        return await callback.answer("⚠️ Голосование не активно.", show_alert=True)
    ...
    if game.votes.get(voter): 
        return await callback.answer("️ Вы уже голосовали.", show_alert=True)
    ...
    # Если проголосовали все живые - досрочно завершаем
...
async def resolve_votes(chat_id: int):
    ...
    game.phase = "day_resolve" # Блокируем повторные вызовы
    
    if not game.votes:
        await bot.send_message(chat_id, "️ Никто не проголосовал. Ничья. Раунд завершен.")
    else:
        counts = {}
